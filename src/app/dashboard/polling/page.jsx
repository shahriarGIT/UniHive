        ...
        <div style={{ marginTop: 20 }}>
          ...
          {isHost && (
            <div style={{ marginTop: 20 }}>
              ...
              {/* Show Options for All Poll Types */}
              {options.map((option, index) => (
                <div key={index}>
                  <input
                    placeholder={`Option ${index + 1}`}
                  ...
                  {pollType !== "true_false" && <span> Correct</span>}
                </div>
              ))}
      ...
      {livePoll && (
        <div style={{ marginTop: 30, padding: 20, border: "1px solid black" }}>
          ...
          >
            <div
              style={{
                width: `${(totalVotes - 1 / usersInRoom.length - 1) * 100}%`,
                height: "20px",
          ...
          {/* {voteResults && (
            <div style={{ marginTop: 20 }}>
              <h3>Live Results:</h3>
              {livePoll.options.map((option, idx) => (
                <div
                  key={idx}
                  style={{
                    color: livePoll.correctAnswers.includes(option)
                      ? "green"
                      : "black",
                  }}
                >
                  {option}: {voteResults[option] || 0} votes
                </div>
              ))}
            </div>
          )} */}
